...
class TestSampleDB(unittest.TestCase):
    ...
    def test_get_study(self):
        study = self.db.create_study("test", "TEST", False, "Max", "No Description")
        other_study = self.db.get_study(study.id)[0]
        self.assertEqual(study, other_study)

    def test_add_study_subject(self):
        study = self.db.create_study("test", "TEST", False, "Max", "No Description")
        study_subject1 = self.db.add_study_subject("1", study.id)
        study_subjects = self.db.get_study_subjects(study.id)
    ...
    def test_add_study_subjects(self):
        study = self.db.create_study("test", "TEST", False, "Max", "No Description")
        uids = ["abc", "def"]
        study_subjects = self.db.add_study_subjects(uids, study.id)
    ...
    def test_get_study_subjects(self):
        study = self.db.create_study("test", "TEST", False, "Max", "No Description")
        study_subject1 = self.db.add_study_subject("1", study.id)
        study_subject2 = self.db.add_study_subject("2", study.id)
        study_subjects = self.db.get_study_subjects(study.id)
        self.assertIn(study_subject1, study_subjects)
    ...
    def test_delete_specimen_type(self):
        specimen = self.db.register_new_specimen_type("DNA")
        self.assertIsInstance(specimen, SpecimenType)
        self.assertIsNotNone(self.db.get_specimen_types())
        self.db.delete_specimen_type(specimen.id)
        self.assertEqual(self.db.get_specimen_types(), [])
    ...
    def test_add_specimen_non_longitudinal(self):
        study = self.db.create_study("test", "TEST", False, "Max", "No Description")
        study_subject = self.db.add_study_subject("1", study.id)
        specimen_type = self.db.register_new_specimen_type("DNA")
        today = date.today()
        with self.db._session_scope() as session:
            specimen = self.db._add_specimen(
                session, study_subject.uid, study.short_code, specimen_type.label, today
            )
    ...
    def test_get_specimens(self):
        ...
        with self.db._session_scope() as session:
            specimen1 = self.db._add_specimen(
                session, study_subject.uid, study.short_code, specimen_type.label, today
            )
            specimen2 = self.db._add_specimen(
                session,
                study_subject.uid,
                study.short_code,
                specimen_type.label,
        ...
        some_specimens = self.db.get_specimens(
            study_subject.uid, study.short_code, today
        )
    ...
    def test_get_specimen(self):
        ...
        with self.db._session_scope() as session:
            specimen2 = self.db._get_specimen(
                session, study_subject.uid, study.short_code, specimen_type.label, today
            )
        ...
        self.assertEqual(
            specimen1,
            specimen2,
            "{} and {} should be equal".format(specimen1, specimen2),
        )
    ...
    def test_add_matrix_plate_with_specimens(self):
        ...
        (
            ...
            specimens,
            matrix_tubes,
        ) = self.db.add_matrix_plate_with_specimens(
            "1", location.id, specimen_entries, True, True
        )
    ...
    def test_update_matrix_tube_locations(self):
        ...
        (
            ...
            "1", location.id, specimen_entries1, True
        )
        (
            matrix_plate2,
            study_subjects,
            ...
            "2", location.id, specimen_entries2, True
        )

        update_list = [
            {"barcode": "1", "plate_uid": "2", "well_position": "B01"},
            {"barcode": "2", "plate_uid": "2", "well_position": "B02"},
            {"barcode": "3", "plate_uid": "1", "well_position": "B01"},
            {"barcode": "4", "plate_uid": "1", "well_position": "B02"},
        ]
        ...
        (
            ...
            specimens,
            matrix_tubes,
        ) = self.db.update_matrix_tube_locations(update_list)
        ...
        update_list = [
            {"barcode": "1", "plate_uid": "2", "well_position": "B02"},
            {"barcode": "2", "plate_uid": "2", "well_position": "B01"},
            {"barcode": "3", "plate_uid": "1", "well_position": "A01"},
            {"barcode": "4", "plate_uid": "1", "well_position": "A02"},
        ]
        ...
        update_list = [
            {"barcode": "1", "plate_uid": "1", "well_position": "A02"},
            {"barcode": "2", "plate_uid": "1", "well_position": "A01"},
        ]

        self.assertRaises(
            IntegrityError, self.db.update_matrix_tube_locations, update_list
        )
    ...
    def test_get_matrix_tube(self):
        ...
        (
            ...
            "1", location.id, specimen_entries, True
        )
    ...
    def test_get_matrix_tubes(self):
        ...
        for tube in matrix_tube_query:
            self.assertIn(tube, matrix_plate.tubes)
