        ...
        try:
            if not study.get("id"):
                ...
                try:
                    ...
                    study, study_subjects, specimens, matrix_tubes = db.update_study(
                        study_id, study
                    )
                except IntegrityError as e:
                    raise InvalidUsage(parse_integrity_error(e), status_code=403)
...
def add_study_subject_file(study_id):
    ...
    try:
        ...
        raise InvalidUsage(
            "File Malformed, should be .csv and header should contain 'UID'",
            status_code=403,
        )
...
def delete_specimen_type(specimen_type_id):
    try:
        ...
        if "FOREIGN KEY" in e.args[0]:
            raise InvalidUsage(
                "Cannot Delete Specimen Type -- Currently In Use", status_code=403
            )
        else:
            raise InvalidUsage(parse_integrity_error(e), status_code=403)
...
def upload_plate():
    ...
    try:
        ...
        (
            ...
            specimens,
            matrix_tubes,
        ) = db.add_matrix_plate_with_specimens(
            plate_uid,
            location_id,
            specimen_entries,
        ...
        raise InvalidUsage(
            "File Malformed, should be .csv and header should contain ['Barcode', 'Well', 'UID', "
            "'Specimen Type', 'Date', 'Study Short Code', 'Comments']",
            status_code=403,
        )
    except IntegrityError as e:
        raise InvalidUsage(parse_integrity_error(e), status_code=403)
        ...
        raise InvalidUsage(
            "File Malformed. Are all study codes valid and specimen types registered?",
            status_code=403,
        )
...
def hide_plates():
    ...
    if to_hide is not None:
        try:
            ...
            raise InvalidUsage(
                "Plate no longer exists. Please reload application.", status_code=403
            )
    else:
        raise InvalidUsage("Invalid API Call", status_code=400)
...
def update_plates():
    ...
    try:
        ...
        raise InvalidUsage(
            "File Malformed, should be .csv, file names should be plate UID, and header should contain"
            " ['Well', 'Barcode', 'Comments']",
            status_code=403,
        )

    except IntegrityError:
        raise InvalidUsage(
            "Tube Position Conflict. Make sure all plates with moved tubes are being updated.",
            status_code=403,
        )
...
def search_for_specimens():
    ...
    try:
        ...
        return send_file(
            temp_path, as_attachment=True, attachment_filename="specimen_search.csv"
        )
    except DateParseError as e:
        raise InvalidUsage(e.message, status_code=403)
    except KeyError:
        raise InvalidUsage(
            "File Malformed, should be .csv, header should contain"
            " ['UID', 'Study Short Code', 'Specimen Type', 'Date']",
            status_code=403,
        )
    except ValueError as e:
        raise InvalidUsage(e.args[0], status_code=403)
...
def search_by_barcode():
    ...
    try:
        search_file = request.files.get("files")
        barcoded_entries, fields, barcode_index = bf.parse_barcode_search_file(
            search_file
        )
        ...
        raise InvalidUsage(
            "File Malformed, should be .csv and header should contain ['Barcode']",
            status_code=403,
        )
    except ValueError as e:
        raise InvalidUsage(e.args[0], status_code=403)
...
def delete_specimens():
    ...
    try:
        ...
        matrix_tube_ids, specimen_ids = db.delete_matrix_tubes_and_specimens(
            matrix_tubes, specimens
        )
        return jsonify(
            data={"matrix_tube_ids": matrix_tube_ids, "specimen_ids": specimen_ids},
            error={},
        )
    except KeyError:
        raise InvalidUsage(
...
def delete_barcodes():
    ...
    try:
        ...
        raise InvalidUsage(
            "File Malformed, should be .csv, header should contain" " ['Barcode']",
            status_code=403,
        )
    except ValueError as e:
        raise InvalidUsage(e.args[0], status_code=403)
